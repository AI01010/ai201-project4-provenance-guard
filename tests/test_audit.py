...
def test_classification_is_written_and_readable(temp_log):
    audit.log_classification("cid-1", "creator-1", "text", "the original body text", RESULT, SIGNALS, "label text")
...
def test_find_classification(temp_log):
    audit.log_classification("cid-2", "creator-2", "text", "the original body text", RESULT, SIGNALS, "lbl")
...
def test_appeal_appends_next_to_original(temp_log):
    audit.log_classification("cid-3", "creator-3", "text", "the original body text", RESULT, SIGNALS, "lbl")
...
def test_read_log_limit(temp_log):
    for i in range(5):
        audit.log_classification(f"c{i}", "u", "text", "the original body text", RESULT, SIGNALS, "lbl")
    assert len(audit.read_log(limit=2)) == 2


def test_corrupted_line_is_skipped(temp_log):
    audit.log_classification("cid-ok", "u", "text", "the original body text", RESULT, SIGNALS, "lbl")
